...
void CLEAR(){
    if(fi != NULL){
        Elemento* no;
        while (fi ->p_inicio != NULL){
            no = fi ->p_inicio;
            fi ->p_inicio = fi ->p_inicio -> prox;
            free(no);
        }
        free(fi);
    }
...
int POP(agenda_p inicio){
    ...
    Elemento *no = fi -> p_inicio;
    
    fi ->p_inicio = fi ->p_inicio -> prox;
    inicio = fi -> p_inicio -> dados;
    if(fi ->p_inicio == NULL)                                       //fila ficou vazia
        fi -> final = NULL;
    free(no);
...
void imprime_Fila(){
    ...
    if (fi ->p_inicio == NULL){
        ...
        scanf("%d", &inicio -> opcao);
        for(no = fi -> p_inicio; no != NULL; no = no -> prox)
